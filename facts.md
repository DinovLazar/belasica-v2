...
- Club founding year: **1922** — **VERIFIED** (owner via Ace, 2026-08-19). Confirmed as the club's founding year; it is the year the archive's own season run and president list already start from. ⚠️ Recorded here as a fact, **not** published as structured data: the site is an unofficial archive, not the club, so `foundingDate` stays out of the JSON-LD (a standing exclusion, unaffected by this confirmation). It is what backs „104 години историја" on „За нас" (2026 − 1922 = 104). _(Prior status: UNVERIFIED — pull from Drive material, confirm with Ace.)_
...
- Ace's public name: **Аце Стојанов** — **VERIFIED** (owner, chat, 2026-08-08). Author of this site's texts and its editor, of the Facebook profile _FK Belasica 1956-2006_, and of the book „ФК Беласица - гордоста на Струмица". Rendered on `/za-nas` from `ABOUT_AUTHOR_PARAGRAPH`. **Clears PL-1.** _(Prior status: UNVERIFIED — deferred by owner to the About-page phase (3.03).)_
- Ace's father: **Томе Стојанов (1953–2020)** — **VERIFIED** (owner, chat, 2026-08-08). Former ФК Беласица player, **328 првенствени натпревари, во 14 сезони**. Rendered on `/za-nas` from `ABOUT_DEDICATION_PARAGRAPH`, with his name linked to `/legendi/tome-stojanov`. **Clears PL-2.** ✔️ Cross-checked and consistent with the archive's own data: `person-tome-stojanov` carries `careerStats.appearances` = **328** and `playingYears` = **1976–1990** (14 seasons), and is ranked #5 in the book. _(Prior status: UNVERIFIED — deferred to 3.03.)_
...
- Domain: **www.belasicahistory.mk** — VERIFIED (owner, chat, 2026-07-31). Rendered as plain text in §1 of `/pravni-informacii`. ⚠️ **Recorded only — the cutover has NOT happened.** `metadataBase`, `robots.ts` and `sitemap.ts` all still hardcode `https://belasica-v2.vercel.app`; moving them is its own phase (3.07 scope explicitly excluded it). _(Prior status: UNVERIFIED — purchased at Phase 3.04.)_
...
### Рангирање — прекинати изедначувања (broken ties)

⚠️ **DERIVED, not stated by Аце** — the one thing on the Играчи ladder that is not his word (2026-08-22, D-RANKS-1).

Аце's list is **competition-style** („1224"): men on the same appearance count share a number and the span behind it is left empty. Four men on **60 настапи** were all „135" and the list resumed at **139**; there were **29 such groups covering 70 of the 162 ranked players**.

At the owner's instruction every ranked player now carries a **unique** rank. Each man in a former tie took his own number **from within the span his group already owned**, ordered **alphabetically** (`localeCompare`, `mk`) — the order the ladder already displayed them in. 41 documents changed.

What is **VERIFIED** (Аце, list of 2026-08-21, via Lazar): the appearance counts, and the _span of numbers_ each tied group holds. Those are untouched — Малинов, Трендов, Јурчевски and Боев still occupy 135–138 between them, exactly as his page has it, and no player's position on the ladder moved.

What is **DERIVED**: which man inside a group takes which number. Alphabetical order is a display convention promoted to stored data — the same class of figure as Коцев's 53rd place above. **One message from Аце replaces it with the real order.** Until then nothing on the site should be read as his ranking _within_ a former tie.

Rank **161 still holds nobody** — reserved for **З. Ивановски** (48 настапи, 2019–2023), whose first name is still owed. That is OV-81, unchanged by this.
