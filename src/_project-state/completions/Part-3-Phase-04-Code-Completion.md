...
3. **⚠️ NEW and live on 10 pages: „Резултати" and „Приказна за сезоната" currently show the same match list twice.** The parallel Cowork pass filled `results` on `1930-31` … `1940-41` by extracting the **leading blocks of `story` verbatim** (their D-3.04C-5, which says so and anticipates the consequence). It left `story` untouched — so on those 10 seasons both fields begin with the identical list. Measured on live content: **72 of the 83 published `story` blocks across those seasons are repeated verbatim in `results`**, and on **9 of the 10**, „Приказна за сезоната" is the whole match list again followed by **exactly one** paragraph of actual narrative. Verified on the preview at `/arhiva/1938-39`: „Резултати" = 16 rows, „Приказна" = the same 16 rows + 1 narrative paragraph.
   **This is a content overlap, not a template bug** — the page renders each field faithfully, and both sections self-omit correctly. **I deliberately did not de-duplicate it in the template**, for the same reason as D-3.04-10: a template that decides which of two published fields to suppress is overruling the record, and a prefix-match heuristic would be exactly the kind of clever-but-brittle rule that misfires the moment a season's two fields legitimately share an opening line.
   **The fix is content and it is small:** trim the leading result blocks from `story` on those 10 seasons, so `results` holds the list and `story` holds the narrative. It belongs to the content pass that created the overlap, and **ISR 60 means the 10 pages correct themselves within a minute of the edit — no code change, no redeploy.** Worth doing before the remaining 86 seasons are filled the same way.
4. **`results` renders correctly otherwise.** One thing to watch: the real transcriptions interleave **group/round labels** („Група Струмица:", „2 коло", „Полуфинале") as ordinary blocks among the match lines, so those labels take the same hairline row treatment as a match. It reads acceptably, but if the content pass later marks them as headings, the renderer already maps every heading style to an `<h3>`.
5. **A tall standings scan makes a long section.** `tablePhoto` is uncapped in height by design (legibility over page length, D-3.04-3): at 1280 the worst case is ~1560px (`1972-73`, a 946×2163 scan). The jump-rail is the mitigation. If the owner finds it too dominant, the single lever is `MattedPhoto`'s `maxHeightRem` on that one call site.
6. **Season titles carry guillemets and one does not** (`1992-93`, D-2.09-4). The H1 therefore reads „Сезона 1985/86“ with quotes. Rendered verbatim by decision (D-3.04-8); one content normalisation of the 96 titles fixes it site-wide with no code change.
7. **Three components are now unimported** (`StandingsTable`, `SquadTable`, `PersonChip`). Kept deliberately (D-3.04-6) and commented. **They should be deleted together with the legacy `finalTable`/`squad`/`trainers` fields when the model re-locks after 3.06** — that is their trigger.
8. **The archive index (`/arhiva`) still ignores `teamPhoto`.** Its card lead is still „earliest free-text `date`" (D-2.03-3), so the index and the season page can show different lead photos for the same season. Out of scope this phase; a one-line GROQ change in a later one, and it is the last place the „curated lead is modelled but not wired" note in `current-state.md` still applies.
9. **The two archive templates now behave differently on jump-nav** (index sticky, season not — D-3.04-5). Intentional, and worth knowing before anyone „fixes" the inconsistency.
10. **One fixture-verified state carries over as an owed item** (§2): the **all-empty season notice**. No published season is fully empty, so it cannot be reached from live content and was proven against a temporary local fixture only. It is a guard, not a page anyone will see — but it is unproven against real data, the same shape as D-2.03-4 / D-2.06-6. (`results` was the other one and it **cleared during the phase**.)
