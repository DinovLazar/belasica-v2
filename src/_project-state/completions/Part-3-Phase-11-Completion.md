...
- **A latent sharp edge found during self-review, inert today: the script's query does not exclude drafts.** `*[_type=="season"]{...}` run **with** a write token returns drafts as well as published documents, and `bySlug` is a `Map` keyed by slug — so if a season ever has an unpublished draft at run time, the map keeps whichever the API returned last, `hasStory`/`hasResults` may be read off the draft, and the patch may land on `drafts.season-<slug>` where **nothing on the site would show it**. **Checked against production with the real token: 96 season documents, 0 drafts** — so a run today is safe and writes to published IDs directly. If a draft is open in Studio when the owner runs `--commit`, that season is the one to re-check. The one-line hardening, if it ever matters, is `*[_type=="season" && !(_id in path("drafts.**"))]`. Not changed here: the phase ships the script as written and makes no untested edit to a tool the owner is about to run.
