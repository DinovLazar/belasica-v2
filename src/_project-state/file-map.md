...
- `src/components/home/SectionOverline.tsx` — section overline: `onPaper` = orange rule + navy text; `onNavy` = orange text on SOLID navy (4.6:1); **`onPhoto` (added 3.03, D-3.03-5)** = orange rule marker + PAPER text, for overlays on a photo+gradient (hero, moment band) where orange text fails AA (Phase 1.05; onPhoto 3.03)
