...
## 2026-07-31 — Phase 3.09-Code (performance & accessibility)

- **One dependency added, dev-only, pinned exact: `lighthouse@13.4.1`** in `devDependencies`. It is the phase's measurement tool, permitted by the brief on exactly those terms. **13.4.1 deliberately matches the version 3.04d used**, so this phase's baseline and the earlier numbers are comparable, and so 3.11 can repeat the run. It ships nothing to the browser and is not imported by any source file. No production dependency was added, upgraded or removed — Next.js 15.5.20, React 19.2.4, Tailwind CSS 4.3.2, `sanity` 4.22.0, `next-sanity` 11.6.13, `@sanity/image-url` 2.1.1, `@vercel/analytics` 2.0.1, Motion 12.42.2, Lucide 1.24.0 all unchanged.
- **`next.config.ts` — one key added:** `images.formats: ["image/avif", "image/webp"]` (D-3.09-4). A **format** change only; `quality` stays at `next/image`'s default 75 on every photograph and D-3.04d-5 is untouched. Verified by content negotiation against the running server: an AVIF-capable client gets `image/avif` at 135,521 bytes where a WebP-only client gets `image/webp` at 150,776 for the same variant. `remotePatterns` and `experimental.staticGenerationRetryCount: 2` are unchanged.
- **`eslint.config.mjs` — ignore patterns anchored with `**/`.** They were root-only (`.next/**`), so a git worktree under `.claude/worktrees/<branch>/` carrying its own `.next` was walked and parsed in full and **`npm run lint` died with a V8 out-of-memory before reaching a single source file**. This was pre-existing on `main`, not introduced here — reproduced on a clean stash. Now `**/.next/**`, `**/out/**`, `**/build/**`, plus `.claude/**` and `_to_delete/**`. `npm run lint` exits **0**.
- **No `brand.md` token changed.** The one measured AA failure (D-3.09-7) was fixed by moving to an existing opacity step, not by editing a token.
- **No `@theme` change, so no new `tailwind-merge` registration was needed** (the D-3.04-1 duty is unchanged: `display, h1, h2, h3, stat, stat-lead, wordmark, body-l, body, small, overline` + `tracking-overline`).
- **No env var added or changed. No schema change, no Sanity write, no content edit.**
- ⚠️ **New local-only build-failure mode, unrelated to the Sanity CDN one (D-3.05a-9).** On **Node 26.3.0 / Next 15.5.20** an *incremental* `npm run build` intermittently dies before compiling with `TypeError: Cannot read properties of undefined (reading 'length')` at `WasmHash._updateWithBuffer` in webpack's bundled `bundle5.js`. **Fix: `rm -rf .next/cache` — not the whole `.next`**; isolated by testing both, the corruption lives entirely in the webpack persistent cache. It never fires on a cold build, so Vercel is unaffected; it is a tax on both operators' machines when iterating. The failing log is ~2 MB because webpack dumps its minified bundle into the stack trace — filter it with `awk 'length($0) < 300' build.log | tail -40` before reading.
- `npm run build` (**270 pages**), `npm run lint` and `npx tsc --noEmit` all exit 0 on this stack.

## 2026-08-02 — Phase 3.11-Code (book extraction data drop + season content fill script)

- **No dependency added, upgraded or removed.** `package.json` and `package-lock.json` are **byte-identical to `main`**. Versions unchanged: Next.js 15.5.20, React 19.2.4, Tailwind CSS 4.3.2, `sanity` 4.22.0, `next-sanity` 11.6.13, `@sanity/vision` 4.22.0, `@sanity/image-url` 2.1.1, `@portabletext/react` 6.2.0, `@vercel/analytics` 2.0.1, Motion 12.42.2, Lucide 1.24.0, `@sanity/client` 7.23.1 (dev), `lighthouse` 13.4.1 (dev).
- **`data/book/` is a tracked source drop, not code.** Seven files, ~6,3 MB, machine-extracted from „ФК Беласица – гордоста на Струмица" (Аце Стојанов, финална верзија **04.10.2025**) — `seasons.json`, `matches.json`, `master-table.json`, `index.json`, `season-content.json`, plus `README.md` and `extraction-report.md`. **Nothing in the application imports it**, so it adds **zero** bytes to any bundle and cannot affect the build; `npm run build` is **271/271**, the same page count as 3.10. It is committed across two commits — `7ef3547` on `main` and this phase's own (D-3.11-5). Every match and squad row carries `sourceLine`, the book's original line verbatim.
- **`scripts/fill-season-content.mjs` — a `setIfMissing`-only content filler, plain Node ESM, no new dependency.** It fills the **existing** `season.story` and `season.results` `block[]` fields; **no schema change, and none is needed.** Every write is `setIfMissing`, so a season that already holds a story or results keeps exactly what it has and a re-run changes nothing. **Dry run is the default** and writes nothing to Sanity — it needs no token, only read access, and emits `docs/ingestion/season-content-report.md`. **`SANITY_API_WRITE_TOKEN` is required only with `--commit`**, read from the environment or from `.env.local` by the script's own parser (no `dotenv` dependency, the 2.09 precedent); without it, `--commit` exits non-zero before touching anything. Flags: `--commit`, `--only <slugs>`, `--field story|results|both`, `--report <path>`. Portable Text `_key`s are **deterministic** — a SHA-1 of slug + field + index + text — so the same input produces the same keys and the patch is idempotent; it commits with `autoGenerateArrayKeys: false` for that reason. Pinned `apiVersion: "2026-07-15"`; project/dataset default to `f8rmnfry/production` and are overridable via `NEXT_PUBLIC_SANITY_PROJECT_ID` / `_DATASET`.
- **No env var added or changed.** `SANITY_API_WRITE_TOKEN` already exists in `.env.example` (empty, documented at 2.09, D-2.09-5); this phase adds a second consumer of it, not a new variable.
- **No `next.config.ts`, `eslint.config.mjs`, `globals.css`, `@theme` or `brand.md` change**, so the D-3.04d-1 `tailwind-merge` registration duty is untouched (`display, h1, h2, h3, stat, stat-lead, wordmark, body-l, body, small, overline` + `tracking-overline`).
- **No schema change and no Sanity write.** `src/sanity/schemaTypes/index.ts` is byte-identical to its pre-phase state; `match` stays unregistered (D-3.11-1). The only Sanity traffic this phase made was the dry run's single read query.
- `npm run build` (**271 pages**), `npm run lint` and `npx tsc --noEmit` all exit 0 on this stack.
- ⚠️ **The 3.09 `WasmHash` build flake fired again and is now confirmed reproducible across phases.** The first `npm run build` of this phase died at `TypeError: Cannot read properties of undefined (reading 'length')` in `WasmHash._updateWithBuffer` before compiling anything. **`rm -rf .next/cache` and a rebuild passed** — the documented fix, and again *not* the whole `.next`. Environment flake on Node 26.3.0 / Next 15.5.20, never on a cold build, so Vercel is unaffected.

## 2026-08-07 — Phase 3.14-Code (four owner-requested label & content corrections)

- **No dependency added, upgraded or removed.** `package.json` and `package-lock.json` are **byte-identical to `main`** (verified with `git diff --stat`). Versions unchanged: Next.js 15.5.20, React 19.2.4, Tailwind CSS 4.3.2, `sanity` 4.22.0, `next-sanity` 11.6.13, `@sanity/vision` 4.22.0, `@sanity/image-url` 2.1.1, `@portabletext/react` 6.2.0, `@vercel/analytics` 2.0.1, Motion 12.42.2, Lucide 1.24.0, `@sanity/client` 7.23.1 (dev), `lighthouse` 13.4.1 (dev).
- **`scripts/fix-andreev-role.mjs` — new, a one-off single-document role correction requiring `SANITY_API_WRITE_TOKEN` (only with `--commit`).** Plain Node ESM, no new dependency; it imports `@sanity/client`, already a dev dependency and already used by `ingest/bios.mjs` and `fill-season-content.mjs`. Same `.env.local` parser as those two (no `dotenv`). **Dry run is the default** and needs only read access; `--commit` without the token exits 1 before touching anything. **`SANITY_API_WRITE_TOKEN` is not a new variable** — it is documented in `.env.example` since 2.09 (D-2.09-5); this is its third consumer. Nothing in the application imports the script, so it adds zero bytes to any bundle.
- ⚠️ **The token is NOT present in `.env.local` on Petar's machine** — that file holds only the three public `NEXT_PUBLIC_SANITY_*` values. The script was therefore dry-run-verified only; the actual patch went through the authenticated Sanity MCP (D-3.14-5).
- **No `next.config.ts`, `eslint.config.mjs`, `globals.css`, `@theme` or `brand.md` change**, so the D-3.04d-1 `tailwind-merge` registration duty is untouched (`display, h1, h2, h3, stat, stat-lead, wordmark, body-l, body, small, overline` + `tracking-overline`).
- **No schema change.** `src/sanity/schemaTypes/` is byte-identical. The Studio field titles are untouched — they label data, not page sections (the D-3.13-1 principle).
